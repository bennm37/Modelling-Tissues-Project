...
ax1.set(title = "Interaction Force")
...
ax2.axis("equal")
ax4.axis("equal")
ax0.set(xlabel="$\delta$",ylabel="$\epsilon$")
ax1.set(xlabel="$r_{ij}$",ylabel="$F_{ij}$")
...
pc = ax0.pcolormesh(P1,EP,phase_diagram,shading="flat",edgecolor="lightgrey",cmap=cmap)
# plt.colorbar(pc,ax=ax0,ticks=NullLocator())
# ax0.set_xticks(p1)
# ax0.set_yticks(ep)
# ax0.grid(which="both")
...
def onclick(event):
    if event.inaxes ==ax0:
        ...
        ax4.set(title = f"Particles at {frame_no}000 time steps")
        a.plot_particles(ax4,0)
        fig.suptitle(f"Delta = {rounded_p1}, Epsilon = {rounded_epsilon}")
